...
//As the data is fetched from the API so the async await is used
const getWeatherData = async () => {
  // Use the try-catch block to handle errors
  try {
    // Create a const that stores the user input from the searchbar or defaults back to 'Dallas' if left blank
    const city = searchInput.value || 'Dallas'

    // Create 2 promises that call the APIs and pass in the city name
    // If the user haven't typed anything, use Dallas as default
    //To get the current weather details
    const currentWeather = new Promise(async (resolve, reject) => {
      try {
        //In the weatherApiData the value for the fetched city will be stored
        ...
        //If it fetches the data then it should print the data
        resolve(await weatherApiData.json())
      } catch (error) { //It will print the error
        reject()
      }
    })
...
const updateDom = data => {
  // Current temperature, using .toFixed to get the 1 place decimal 
